...
## 11. CI 버전 매트릭스에서 시드 생성이 `422 invalid milestone`로 깨짐

**증상** — 로컬·단일 잡에선 통과하던 seeded 테스트가 CI에서 **`3.13` 잡만** 실패했다(`3.12`는 성공). 시드 이슈 생성이 `422 {"field":"milestone","code":"invalid"}` — 방금 만든 마일스톤 번호가 "유효하지 않다"는 응답이었다.

**원인** — `3.12`·`3.13` 매트릭스 두 잡이 **같은 샌드박스 레포(`qa-sandbox`)에 동시 실행**된다. 초기 seed fixture는 라벨·마일스톤을 **고정 이름**으로 만들고 "있으면 지우고 다시 생성"(delete-if-exists)했다. 그래서 한 잡이 만든 마일스톤을 다른 잡의 stale 정리 로직이 **삭제**해 버렸고, 먼저 시작한 잡은 이미 사라진 마일스톤 번호로 이슈를 만들려다 422를 맞았다. 개선 백로그에 적어둔 "공용 샌드박스 경합" 리스크가 실제로 터진 것.

**해결** — seed가 만드는 **모든 리소스 이름에 run-unique 접미사**(`seed-bug-<suffix>`, `seed-milestone-<suffix>` 등)를 붙이고, **delete-if-exists 정리를 제거**했다. 유니크 이름은 사전 충돌이 없어 정리가 불필요하고, 무분별한 delete야말로 동시 실행 중인 형제 잡의 데이터를 지우는 원인이었다. teardown은 각 실행이 만든 고유 리소스만 회수한다. (로컬에서 seeded 스위트를 2개 프로세스로 동시 실행해 재현·검증 완료. 4회차 리뷰의 마일스톤 `per_page=100` 보강은 stale 정리 자체가 사라져 무의미해져 함께 제거.)
