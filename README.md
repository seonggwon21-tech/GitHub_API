...
> — 총 **66 TC** · READ + **write-path(CRUD)** · GitHub Actions CI + Allure Pages 자동 배포
...
| 테스트 | 결과 | 검증 범위 | CI | 리포트 |
|:---:|:---:|:---:|:---:|:---:|
| **66 TC** (User 12·Repos 16·Issues 11·CRUD 27) | **63 passed · 3 skip · 0 fail** | 상태코드·스키마·정합성·**라이프사이클 CRUD**·negative | **GitHub Actions** | **Allure Pages** |
...
| API 검증 | 상태 코드 · 스키마 필드/타입 · 정합성 · 페이지네이션 · **라이프사이클 CRUD**(이슈·코멘트·라벨·마일스톤) · negative(404·422·인증 스코프) |
...
pytest -m write         # write-path CRUD 27개 (repo 스코프 PAT 필요, 없으면 자동 skip)
...
| 문서 | 내용 |
|---|---|
| [학습 가이드](docs/STUDY.md) | 각 파일이 왜 있는지·핵심 개념 5개·예상 면접 질문 (프로젝트 소화용) |
...
| [트러블슈팅](docs/troubleshooting.md) | 구축·실행 중 실제로 막혔던 문제와 해결 과정 (9건) |
